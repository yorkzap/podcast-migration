...
def reformat_iframe(iframe_content):
    ...
    new_content = new_content.replace('\\', '')  # Remove backslashes
    return new_content

def replace_libsyn_iframe(current_content, libsyn_iframe_regex, redcircle_iframe):
    updated_content = re.sub(libsyn_iframe_regex, redcircle_iframe, current_content, flags=re.DOTALL)
...
def main():
    ...
    # Regex pattern to match Libsyn iframe. This may need to be adjusted to match the exact pattern.
    libsyn_iframe_regex = r'<iframe[^>]+src="https?://play\.libsyn\.com/[^"]+"[^>]*></iframe>'
    ...
    for count, post in enumerate(posts, start=1):
        ...
        updated_content = replace_libsyn_iframe(current_content, libsyn_iframe_regex, formatted_iframe)
